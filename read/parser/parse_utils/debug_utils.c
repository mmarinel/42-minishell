...
/*   Updated: 2022/08/11 18:00:56 by mmarinel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	printf(GREEN "[cmd_name: %s, cmd_arg: %s, infile: %s, outfile: %s]" RESET,
		node->content->simple_cmd.cmd_name,
		node->content->simple_cmd.cmd_args,
		node->content->in_redir.file_name,
		node->content->out_redir.file_name
		);
}
...
void	print_env_statement(t_tree_node *node)
{
	if (node->content->env_decl.set)
		printf(GREEN "[EXPORT : infile: %s, outfile: %s " RESET,
			node->content->in_redir.file_name,
			node->content->out_redir.file_name
			);
	else
		printf(GREEN "[UNSET : infile: %s, outfile: %s " RESET,
			node->content->in_redir.file_name,
			node->content->out_redir.file_name
			);
}
	...
	if (subtree_root)
	{
		...
		else if (subtree_root->content->content_type == SIMPL_CMD)
		{
			ft_printf("(cmd: %s; args: %s)",
				subtree_root->content->simple_cmd.cmd_name,
				subtree_root->content->simple_cmd.cmd_args);
		}
